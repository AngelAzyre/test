...
export default new class Nyaa extends AbstractSource {
    ...
    /**
     * @param {import('./types').NyaaElement[]} nodes
     * @param {boolean} batch
    ...
    map (nodes, batch = false) {
        return nodes.map(item => {
            ...
            return {
                ...
                accuracy: (item.querySelector('nyaa\\:trusted')?.textContent?.includes("Yes") || item.querySelector('nyaa\\:remake')?.textContent?.includes("Yes")) ? 'medium' : 'low',
